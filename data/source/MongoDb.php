...
class MongoDb extends \lithium\data\Source {
	 ...
	 *            [`MongoClient::__construct()`](http://www.php.net/manual/en/mongo.construct.php)
	 *            for more information. Defaults to `false`, meaning no persistent connection is
	 *            made.
	 ...
	 *        - `'replicaSet'` _boolean_: See the documentation for `MongoClient::__construct()`.
	 *        	Defaults to `false`.
	 *
	...
	public function disconnect() {
		if ($this->server && $this->server->getConnections() != array()) {
			$this->_isConnected = false;
	...
	public function create($query, array $options = array()) {
		$defaults = array('w' => 1, 'fsync' => false);
		$options += $defaults;
		$this->_checkConnection();
	...
	public function update($query, array $options = array()) {
		$defaults = array('upsert' => false, 'multiple' => true, 'w' => 1, 'fsync' => false);
		$options += $defaults;
		$this->_checkConnection();
